...
public interface OodleLibrary extends Library {
	 ...
	 */
	int OodleLZ_Compress(int codec, OodleLibrary.uint8 src_buf, long src_len, OodleLibrary.uint8 dst_buf, int level, Pointer opts, long offs, long unused, Pointer scratch, long scratch_size);
	/**
	 ...
	 */
	int OodleLZ_Decompress(OodleLibrary.uint8 src_buf, int src_len, OodleLibrary.uint8 dst, long dst_size, int fuzz, int crc, int verbose, OodleLibrary.uint8 dst_base, long e, Pointer cb, Pointer cb_ctx, Pointer scratch, long scratch_size, int threadPhase);
